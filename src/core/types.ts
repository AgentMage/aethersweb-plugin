...
export interface SpinPayload {
	...
	/**
	 * Self-declared identity of the agent that produced this write, e.g. "claude-opus-5". Absent
	 * means a person did it.
	 *
	 * `source` already records *how* a change was witnessed (observed vs detected); it says nothing
	 * about *who* made it, and an agent authoring a file is `observed` exactly like a person typing
	 * in Obsidian. This is the axis that separates them. It matters most for formats that cannot
	 * carry an inline signature — a JSON or binary file written by an agent is attributed here or
	 * nowhere at all.
	 */
	authored_by?: string;
